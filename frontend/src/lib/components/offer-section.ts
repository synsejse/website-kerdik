...
function initMap(offers: OfferSummary[]) {
    ...
    try {
        ...
        const avgLat =
            offersWithCoords.reduce((sum, o) => sum + (o.latitude || 0), 0) /
            offersWithCoords.length;
        const avgLng =
            offersWithCoords.reduce((sum, o) => sum + (o.longitude || 0), 0) /
            offersWithCoords.length;
        ...
        setTimeout(() => {
            ...
            offersWithCoords.forEach((offer) => {
                if (offer.latitude == null || offer.longitude == null || !map)
                    return;
...
export function initOfferSection(): void {
    ...
    function createOfferElement(offer: OfferSummary) {
        ...
        article.className =
            "bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-xl hover:border-primary/20 hover:-translate-y-1 flex flex-col";
        ...
        titleLink.className =
            "text-inherit no-underline hover:text-primary transition-colors";
        ...
        descDiv.id = descId;
        descDiv.className =
            "m-0 text-sm text-gray-600 leading-relaxed line-clamp-3 transition-all duration-300 prose prose-sm max-w-none";
        ...
        if (needsExpansion) {
            ...
            expandBtn.className =
                "text-sm font-medium text-primary hover:text-blue-600 transition-colors mt-2";
        ...
        a.className =
            "inline-flex items-center gap-2 text-sm font-bold text-primary hover:text-blue-600 transition-colors no-underline";
        a.href = detailUrl;
    ...
    `;
        ...
        // Clone icon template
        const chevronTemplate = document.getElementById(
            "icon-chevron-right",
        ) as HTMLTemplateElement;
        if (chevronTemplate) {
            const chevronIcon = a.querySelector(".icon-chevron-right");
            if (chevronIcon) {
                chevronIcon.replaceWith(
                    chevronTemplate.content.cloneNode(true),
                );
            }
        }
        ...
        if (offer.link && offer.link.trim() !== "") {
            ...
            externalLink.className =
                "inline-flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-gray-900 transition-colors no-underline";
    ...
    function showEmptyState() {
        if (container) {
            container.innerHTML =
                '<div class="col-span-full text-center py-12"><p class="text-gray-600">Žiadne ponuky na zobrazenie.</p></div>';
        }
    }
