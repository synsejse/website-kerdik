...
export function initAdminSetup(): void {
  ...
  const submitBtn = document.getElementById("setup-btn") as HTMLButtonElement | null;
  const errorMessage = document.getElementById("error-message") as HTMLDivElement | null;

  if (!form || !usernameInput || !passwordInput || !confirmInput || !submitBtn || !errorMessage) {
    return;
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Nepodarilo sa dokončiť prvotné nastavenie.";
