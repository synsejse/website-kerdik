import { api, type AdminStatus, type AdminUser, type AdminUserInvite } from "../api";
...
class UsersPageController {
  ...
  private bindEvents(): void {
    ...
    this.elements.inviteUserBtn?.addEventListener("click", () => this.openInviteModal());
    ...
    this.elements.inviteModalClose?.addEventListener("click", () => this.closeInviteModal());
    this.elements.inviteModalCancel?.addEventListener("click", () => this.closeInviteModal());
    this.elements.inviteForm?.addEventListener("submit", (event) => this.handleInviteSubmit(event));
  ...
  private async loadData(): Promise<void> {
    const { loading, container, invitesContainer, noUsers, noInvites } = this.elements;
  ...
  private async deleteInvite(id: number): Promise<void> {
    showConfirmDialog("Naozaj chcete zrušiť túto pozvánku?", async () => {
      try {
        ...
        alert(error instanceof Error ? error.message : "Chyba pri rušení pozvánky.");
      }
    });
  }
  ...
  private async copyInviteLink(id: number): Promise<void> {
    ...
    const inviteUrl = new URL(invite.invite_path, window.location.origin).toString();
  ...
  private async handleInviteSubmit(event: Event): Promise<void> {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "Chyba pri vytváraní pozvánky.");
    }
  }
}
