...
export class OffersPageController {
    ...
    private setupEventListeners(): void {
        ...
        offerLatitude?.addEventListener("input", () =>
            this.updateMapFromInputs(),
        );
        offerLongitude?.addEventListener("input", () =>
            this.updateMapFromInputs(),
        );
    ...
    private editOffer(id: number): void {
        ...
        if (offerExcerpt)
            setMarkdownEditorValue(offerExcerpt.id, offer.excerpt || "");
        if (offerContent)
            setMarkdownEditorValue(offerContent.id, offer.content || "");
        if (offerLink) offerLink.value = offer.link || "";
        if (offerLatitude)
            offerLatitude.value = offer.latitude ? String(offer.latitude) : "";
        if (offerLongitude)
            offerLongitude.value = offer.longitude
                ? String(offer.longitude)
                : "";
    ...
    private async applyCrop(): Promise<void> {
        ...
        const { imageCropContainer, imagePreview, imagePreviewImg } =
            this.elements;
                    ...
                    (blob: Blob | null) => {
                        if (blob) {
                            ...
                            if (imageCropContainer)
                                imageCropContainer.classList.add("hidden");
    ...
    private openModal(): void {
        const { modal, offerId, form, offerExcerpt, offerContent } =
            this.elements;
